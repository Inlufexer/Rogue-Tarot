...
@export var _start : Vector2i = Vector2i(7, 5)
...
func _generate_branches() -> void:
	var _branches_created : int = 0
	var candidate : Vector2i
	while _branches_created < _branches and _branch_candidates.size():
		candidate = _branch_candidates[randi_range(0, _branch_candidates.size() -1)]
		if _generate_critical_path(candidate, randi_range(_branch_length.x, _branch_length.y), str(_branches_created + 1)):
			_branches_created += 1
		else:
